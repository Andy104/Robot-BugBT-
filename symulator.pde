...
class Robot {
  ...
  void CollisionDetection(PVector posL, PVector posR) {
    for (int i = 0; i < objectTab.getRowCount(); i++) {
      ...
      if (((int)posR.x <= row.getInt("x")) && ((int)posR.x >= row.getInt("x")-25)) {
        //println("czułka prawa", row.getInt("x"), (int)posR.x);
        dL.x = a.x+10;
      } else {
        dL.x = a.x+20;
      }
      
      if (((int)posR.y <= row.getInt("y")) && ((int)posR.y >= row.getInt("y")-25)) {
        println("czułka prawa", row.getInt("y"), (int)posR.y);
        dL.y = a.y+10;
      } else {
        dL.y = a.y+12;
      }
      
      if (((int)posL.x >= row.getInt("x") - 25) && ((int)posL.x <= row.getInt("x") + 25)) {
        println("czułka lewa ", row.getInt("x"), row.getInt("y"), (int)posL.x, (int)posL.y);
        dR.x = row.getInt("x") - (int)posL.x;
      } else {
        dR.x = a.x+20;
      }
      if (((int)posL.y >= row.getInt("y") - 25) && ((int)posL.y <= row.getInt("y") + 25)) {
        dR.y = row.getInt("y") - (int)posL.y;
      } else {
        dR.y = a.y+12;
      }
    }
